  ...
  const generateTranscript = async (e: any) => {
    ...
    aaiSocket.current.onmessage = voicePrompt => {
      ...
      if (res.message_type === 'FinalTranscript') {
        console.log('FINAL TRANSCRIPT.  DO STUFF HERE.', msg)
        sendMessage()
        msg = ''
        return
      }

      onTranscript({ target: { value: msg } } as any)
    }
    ...
    aaiSocket.current.onclose = event => {
      ...
      if (aaiSocket) {
        aaiSocket.current = null
      }
    }

    aaiSocket.current.onopen = () => {
      if (!aaiSocket.current) return
      ...
      navigator.mediaDevices
        ...
        .then(stream => {
          ...
          recorder.current &&
            'startRecording' in recorder.current &&
            recorder?.current?.startRecording()
        })
